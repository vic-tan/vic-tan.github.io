---
layout: page
title: "任务"
header-img: "img/orange.jpg"
---

## -----------------------------------2024 年工作清单----------------------------





## -----------------------------------2023 年工作清单----------------------------


***
### 2023 HDMI 认证
***

|机芯 |型号 |  
| :----- | :---- |
| ATV      | 土耳其 Arcelik 98D8100 | 
| 41A GTV  | HDMI   | 
| 2885N    | TVY23-A-002 65D8100   | 

***
### 2023年09月
***

> +
> + 
> + 
> + 
> + 
> + 
> + 
> + 
> + 
> + 
> + 0148156: 【41A/PID117】【CVT问题】板卡出现概率性切通道后，声音频率在上一个通道的频率，通道名和画面是正常。比例约10%
> + 0148157: 【41A/PID117】【CVT问题】DTV通道概率性无图，需要多次切换通道，比例约10%
> + 0148218: 51M GTV_中美Imperial客户_ ID370/371 _ATSC制式_预置泰霖频道后无台
> + 0148252: 串口互切Tconless和vby1 屏参和眼图文件的指令执行成功，但画面无作用
> + 0148143: 打开内置pattern后，执行灰度白场指令无作用，机器闪退出pattern模式 

***
### 2023年08月
***


> + 0147804: 【51M/土耳其A客户】评估HDMI CEC开关默认值从on改为off该客户需求可行性以及开发周期
> + 0146005: [ID:139] 泰霖反馈抄完KEY 后断电上电 ，发现P模式被关闭。
> + 0146976: 【85N-GTV/北非B客户ID:13 】升级软件或已登录google帐号，进行shop，断电上电在开机向导没有home和shop模式选择的菜单
> + 0145738: 【功能需求】41A /51M ATV/GTV 2885N 项目，各信源，提供 soc获取指定xy坐标的RGB值的接口
> + 生产问题：GTV 41A SHOP 显示红屏
> + 0147224: 【51M_ATV】供应商反馈工厂生产指令切通道，会直接退出工厂测试模式
> + 预制频道不支持的制式未清除上一次的频道
> + 【新需求】Tconless 屏不切ID，可以通过串口替换Vb1和Tconless的眼图参数和屏参
> + 0146568: 切ID 方法未有返回成功失败状态，需要同步返回状态
> + 0147485: 【RT2851 DVB 新西兰 松下ID:306】进入工厂菜单查看软件版本信息，版本不显示。
> + 0147487: 【RT2851 DVB 新西兰 松下ID:306】进入工厂菜单查看Fingerprint信息，品牌显示MICROMAX,，实际测试是PANASONIC
> + 进入ATV/AV/HDMI 直接闪退，Google Reset 和升级软件也没法恢复。
> + 0146482: 【85N-GTV/北非B客户/ID:14】USB强制升级到V90.0001版，查看工厂白平衡数据都是128
> + 0146568: 切ID 方法未有返回成功失败状态，需要同步返回状态
> + 0146366: 【RT2841A 阿塞拜疆 YOSHIRO 】生产问题 ：切通道会跳主页
> + 0146204: 用串口指令设置音量80后插入耳机，此时耳机音量只有25，拔插几次后耳机音量才到80


***
### 2023年07月
***


> + 
> + 
> + 
> + 
> + 
> + 
> + 0147852: 【2841A GTV】id:132产线shop 时会卡在红屏界面，不显示绿屏
> + 0145825: ID18,制式colmbia，惠州预制频道缺少ATV_cable 29台
> + 0145826: ID3，制式ISDB，工厂预制频道TL，5.1无法播放（DTV）无声无像
> + 0145815: ID:30, 制式：ATSC 预制频道CVT工厂，antenna _atv缺少频道14，多出62.0
> + 0145821: ID12，DVB制式，泰林工厂预制频道cable缺少频道800(DTV)
> + 0144728: 【RT2841A泰国工厂松下客户PID：130】泰国工厂产线上报问题：TV插入USB播放mp3音乐或者mp4视频时，弹出Unsupported media file format. Play the nextmusic.且无声音，画面正常
> + 0145980: 工厂指令写入的白平衡数据和读取出来的值不一样
> + 0145932: 【85N-GTV/约旦G客户/ID:17】：工厂菜单的Picture Mode对比设置的Picture Mode缺少ECO和Vivid模式
> + 0145820: 【51M/土耳其A客户/PID690.691】HDR10+认证测试，CEC&EDID测试问题  4-2-4测试Fail/4-2-10的测试Fail
> + 0145747: 【85N-GTV/北非B客户/ID:13】升级完软件，google复位完，查看工厂菜单开机模式是STB，断电再上电后，直接开机
> + 0145636: 软件U盘OTA 升级后工厂遥控器部分按键无反应
> + 0145688: shop复位后会清除之前设置好的白平衡数据
> + 0145500: 【51M-GTV/0705量产分支 乌兹别克ARTEL项目/ ID:315】NVM复位后，电视有时在主页面，有时会出现搜台界面，有时会闪现以下提示：The input doesn't support auto-scan
> + 0145431: 【V1230711，ID184】白平衡预值后，使用串口读取值与写入值不一致
> + 0145309: 工厂菜单手动调节白平衡数值时，只有当前通道生效
> + 0145419: 插入CI发送检测卡套指令后，再切换信源到HDMI1执行获取当前信源指令，获取失败
> + 0145229: 【V8-GT51MGL-001230704 ID240】任一信源下，工厂串口指令不能跳转到卫星
> + 0145228: 【V8-GT51MGL-001230704 ID240】任一信源下，工厂串口指令不能跳转到DTV
> + 0145142: 【51M/土耳其A客户/PID691】DVB-S没有信号
> + 0145144: 【51M/土耳其A客户/PID691】每次切通道都会跳回主页，需要二次切
> + 0145043: 【51m_GTV/印度acer /ID 294】预置频道表6后，切ATV通道，概率性获取通道名为空，需要断电重新测试 
> + 土耳其 Arcelik 98D8100 HDMI

***
### 2023年06月
***

> + 143673	在P模式下做usb升级无法跳过开机向导---主要Block项
> + 143751	HDMI3发串口获取当前信源返回的是HDMI1
> + 144119	SHOP复位后，在未到finnish绿屏页面时执行断电上电操作后，电视死机
> + 143750	工厂指令HDMI3信源下串口切不到HDMI2
> + 144606	id:11 试产线反馈测试串口远场语音、led灯无作用
> + 144002	在HDMI 1信源下播放音频，用串口指令检测按键板后无声
> + 144365	在DTV/ATV 上播放有声音的信源，此时开老化，按power 键退老化后，DTV/ATV 没有声音
> + 144286	工厂菜单Other Option-->Watch Dog选项，不能设置为off
> + 143756	工厂指令执行预制频道动作后画面概率跳会主页， 然后电视会无声(必现问题)---向毅确认
> + 143758	工厂指令切换spdif后播放无声音输出--wade
> + 143752	无法进行CVT工厂ATSC制式预制频道的操作
> + 143879	进入ATV或 DTV等节目后，无法按上下键进行切台
> + 142980	[id:8] 工厂遥控协议为RCA ,按工厂遥控器TV 、RATTLE、CABLE 无反应
> + 142539	按遥控键发关遥控测试串口，未正确返回遥控码值
> + 142533	P模式下切完ID 会重新找开机向导流程
> + 142532	串口切ID 会自动重启
> + 142662	USB升级软件后会清除电视的key
> + 142543	串口检测CI 卡状态失败
> + 142542	按键测试模式按控制面板未发送对应的广播
> + 142538	串口无法切声音news模式
> + 142537	串口切内置显示黑屏
> + 142528	工厂菜单 PQ Version 版本显示不正确
> + 142527	Factory 显示软件版本为null
> + 142500	【85N-GTV/中东A客户/ID:11】进入图像模式-打开对比度菜单，按6次OK键，出现System和Hotel菜单
> + 142405	key抄写完成后无作用
> + 142503	【85N-GTV/中东A客户/ID:11】进入图像模式-打开对比度菜单，按6次OK键，Hotel菜单的功能未做完整
> + 142505	【85N-GTV/中东A客户/ID:11】进入图像模式-打开对比度菜单，按6次OK键，System菜单的IC Name显示为RT2885
> + 142497	【85N-GTV/中东A客户/ID:11】进入图像模式-对比度，按1950，不能进入到工厂菜单
> + 142501	【85N-GTV/中东A客户/ID:11】进入图像模式-打开对比度菜单，按6次OK键，System和Hotel菜单前面的图标相同
> + 142499	【85N-GTV/中东A客户/ID:11】进入图像模式-打开对比度菜单，按6次OK键，不能进入到工厂菜单
> + 142498	【85N-GTV/中东A客户/ID:11】进入图像模式-对比度，按9735，不能进入到工厂菜单
> + 143196	没有抄写RMCA key也可以完成复位
> + 142809	图像参数设置指令soft(movie)设置失败
> + 142874	工厂遥控器L.SR键无作用
> + 142873	工厂遥控器3D键无作用
> + 142865	工厂遥控器WIFI键无作用
> + 142662	USB升级软件后会清除电视的key
> + 143014	WiFi data read 指令执行返回超时（超3秒），判定失败
> + 142780	白平衡调试工厂指令无作用
> + 142778	内置PATTERN选择指令无作用
> + 142837	按键板检测执行指令后点击面板按键没有显示白场
> + 142835	只能使用USB，且在moka工厂抄写的key，RTK工厂页面无法显示，
> + 142758	串口指令进入老化模式后，遥控器返回键可退出老化
> + 142759	预制惠州频道无作用
> + 142760	Personal音效设置指令实际设置为Standard模式
> + 142834	切project id后，电视自动重启并进入老化模式
> + 142781	widevine KEY检测&读取执行指令失败
> + 142871	USB无法识别出音乐和图片文件
> + 142405	key抄写完成后无作用
> + 142542	按键测试模式按控制面板未发送对应的广播
> + 142981	[id:8] 工厂遥控协议为RCA ,按工厂遥控器3D无反应
> + 142980	[id:8] 工厂遥控协议为RCA ,按工厂遥控器TV 、RATTLE、CABLE 无反应
> + 142543	串口检测CI 卡状态失败
> + 143223	White Balance init 工厂菜单白平衡初始化，只初化当前色温，未全部初如化
> + 143311	SHOP 会弹出蓝牙匹配界面
> + 【文档】U盘抄KEY 设计文档
> + 0144506: 【85N-GTV/北非B客户/ID:13】工厂菜单，User ID显示unknown
> + 0144606: id:11 试产线反馈测试串口远场语音、led灯无作用
> + 【专利】智能U盘串口指令自动发送执行系统
> + 【专利】一种用于批量设置酒店电视机参数的克隆方法及云端系统
> + 【文档】Factory 酒店模式设计规范
> + 【文档】2841A&2851M预置频道列表 （https://rd-mokadisplay.tcl.com/kms/pages/viewpage.action?pageId=74159472）
> + 【文档】Factory支持的串口指令汇总 （https://rd-mokadisplay.tcl.com/kms/pages/viewpage.action?pageId=74186640）
> + 0143760: 「V8-GT51MGL-0010603.0001 ID:139」写入白平衡后，工厂菜单上第一次查看白平衡值未更新
> + 0143764: 「V8-GT51MGL-0010603.0001 ID:139」key抄写返回值概率性错误
> + 0143744: 【生产适应性问题】音频输出功能设置(0x9F 1D) 指令执行成功，实际功能有响应，UI无变化
> + 0143759: 「V8-GT51MGL-0010603.0001 ID:139」在HDMI1信源预制CVT和hz，DTV播放无声无像
> + 0143444: 【51M/ATSC/ID243】RTK UI MX预置频道数据导入
> + 0143477: 【51M/ATSC/ID243】RTK UI CVT预置频道数据导入
> + 0142988: 【51M/ATSC/ID244】HZ_ODM_RTKUI预置频道导入
> + 0144301: 【51M/ATSC/ID243】RTK UI TL预置频道数据导入
> + 0143021: 【51M/ATSC/ID432】HZ_ODM_MOKA_UI预置频道导入
> + 0143434: 【51M_GTV】HDMI1信源下读取信源名，返回值发生变化


***
### 2023年05月
***

> + 0142405: key抄写完成后无作用
> + 0142505: 【85N-GTV/中东A客户/ID:11】进入图像模式-打开对比度菜单，按6次OK键，System菜单的IC Name显示为RT2885
> + 0142501: 【85N-GTV/中东A客户/ID:11】进入图像模式-打开对比度菜单，按6次OK键，System和Hotel菜单前面的图标相同
> + 0142500: 【85N-GTV/中东A客户/ID:11】进入图像模式-打开对比度菜单，按6次OK键，出现System和Hotel菜单
> + 0142499: 【85N-GTV/中东A客户/ID:11】进入图像模式-打开对比度菜单，按6次OK键，不能进入到工厂菜单
> + 0142494: 【51M/印度Acer/ID:220】在ATV信源，工厂信号下任意频道，按面板按键，节目会不停地切换。
> + 0142263: 【51M/欧洲P客户/ID:178】酒店模式下TV to USB卫星频道，Antenna搜台，在Antenna信源下进入酒店模式USB to TV，Antenna显示黑屏
> + Factory Uart现有能支持的串口汇总
> + 2885n R+ HDMI 认证
> + 0141852: [RT2851M/日本I客户/ID:750]Shop复位不恢复无操作自动待机的默认值
> + 0141841: 【51M-GTV/P客户】Xtreme工厂频表错误
> + 0139955: 【V0324，ID135】工厂模式下预制BRANDT频道，TV依旧保持预制前状态
> + 0140821: 【51M/台湾P客户/ID:146】酒店模式设置开机在ATV29台，断电上电，开机后有时在ATV信源，有时仍在原先的信源
> + 0141453: 【RT2841A 中东ALHAFIDH】售后问题 ：每次断电上电后，工厂菜单的running time，会增加15min
> + 0121824: 松下新西兰区域（TVN21-C-067）H3-43D830H8-NZ0项目软件需要添加远场语音串口查询指令



***
### 2023年04月
***

> + 0141229: 【51M GTV/哥伦比亚】墨西哥工厂预制频道添加ID验证:ID277
> + 0140429: 【41A/ATSC/ID117】【生产适应性问题】执行获取热点蓝牙强度的串口命令，TV弹窗提示是否开启定位(GTV普遍现象)
> + 0141142: 【V10079，ID184】未抄写HDCP KEY，HDMI下用22294测试HDCP协议正常
> + 0141064: 【51M/台湾P客户/ID:146】打开酒店模式，设置开机信源后，断电上电，开机后会先出现主页面，再转到设置的信源
> + 0140976: 【51M GTV/CVTE工厂频道导入】中美哥伦比亚CVTE工厂DVB-6M工厂频道导入
> + 0140829: 【51M/ATSC/ID244】CVT预置频道导入
> + 0140837: ID:244，版本：8-GT51MGL-0010413
> + 0140789: 【41A/欧洲P客户/ID:503】在antenna信源有信号的情况下使用酒店模式中USB to TV，导入频道后antenna黑屏，切台恢复
> + 0140342: 【2851M/埃及战略P客户/ID: 】增加埃及MTI功能预置频道
> +	0140312: 【51M/主干软件V0010408-MASTER 哥伦比亚ID:638 台湾P客户/ID:146】运行酒店模式中的USB to TV后，电视黑屏
> + 0140291: 【51M/主干软件V0010408-MASTER 台湾P客户/ID:146】酒店模式设置开机在ATV29台，概率性出现不生效
> + 0139980: 【41A GTV/墨西哥pioneer/ID:117】国家为墨西哥，TV通道将CC功能打开，工厂菜单进行NVM RESET和Shop复位，CC功能依然为开状态，并且CC的显示状态都不进行复位
> + 0139955: 【V0324，ID135】工厂模式下预制BRANDT频道，TV依旧保持预制前状态


***
### 2023年03月
***
> + 0139419: 【RT2851M/RT2841A台湾客户PID：145/146/147/148/149】打開酒店模式，開機，信號源設定在HDMI 1，斷電後隔一下再上電、開機，無信號
> + 0138780: [EU][OTA] Export hotel.bin from user menu Export to USB
> + 0138541: 【2851M/越南Casper/ID:198】NVM reset后，系统没有自动重启。（见一次）
> + 0139115: 【北非B客户/41AGTV/ID:112】工厂NVM复位后一直在绿屏界面，提示nvm is running
> + 0139062: 【台湾松下客户场测问题】工厂菜单shop之后，开机没有进主页
> + 0138350: [RT2841A/中美N客户/ID:348] shop复位后设置菜单里锁功能数据没有清除
> + 0138575: 【51M-GTV/印度P客户/ID:138】ARC接松下B100功放，TV待机再开机，声音从电视输出，查看output显示HDMI-ARC
> + 0138276: 【墨西哥Cobia/2851M/ID:】HDMI3个端口都无法输出图像声音。
> + 0138223: 【51M GTV/哥伦比亚K客户/ID:210】ARC连接Onkyo HT-R558功放，TV待机开机，功放不跟随开机
> + 0138246: 【EU p客户 41A51M PID 501】酒店模式快捷命令9715无法唤出酒店模式菜单
> + 0138111: 51M HDMI CEC问题
> + 0138089: 【乌兹别克ARTEL/2851M/ID:746】HDMI2端口连接22293仪器及DVD等外接设备时，显示无信号。
> + 0138014: 【2841A/香港战略P客户/ID: 298】工厂生产适应性测试 - 工厂遥控器检测NG
> + 0138013: 【2841A/菲律宾P客户/ID:489】工厂生产适应性测试 - 工厂遥控器检测TV(11、10)频道无响应
> + 0138030: [RT2841A/菲律宾P客户/ID:489]电视连接ARC功放后，按下电视遥控器音量加/减按键，按两次才会起作用
> + 0137924: 【51M GTV/SS客户/ID:221】预制频道导入


***
### 2023年02月
***
> + GTV 2841A HDMI 认证EDID 问题修改
> + 0137581: 【51M GTV/欧洲SS客户/ID:221】国家为法国，ANTENNA通道下，播放法国流，HDMI1接SOUNDBAR，出现声音断断续续，卡顿的现象。
> + 0137534: 【51M GTV/欧洲SS客户/ID:221】电视端CEC功能都为开的状态，HDMI3口接CEC碟机，HDMI1口接eARC功能的soundbar，HDMI3通道下关机后，碟机开机或者电视端开机，信源在主页界面下
> + 0137536: 【51M GTV/欧洲SS客户/ID:221】电视端CEC功能都为开的状态，HDMI3口接CEC碟机，HDMI1口接eARC功能的soundbar，HDMI3通道下关机后，电视端开机，DVD不跟随唤醒
> + 0137537: 【51M GTV/欧洲SS客户/ID:221】电视端CEC功能都为开的状态，HDMI3口接CEC碟机，HDMI1口接eARC功能的soundbar，HDMI3通道下关机后，电视端开机，按source键切换信源至HDMI3通道，DVD不能被唤醒。
> + 0137538: 【51M GTV/欧洲SS客户/ID:221】当前通道在非HDMI通道，CEC功能设置为打开状态，切换信源至接有CEC功能碟机的HDMI通道，DVD不能被唤醒
> + 0137539: 【51M GTV/欧洲SS客户/ID:221】当前通道在非HDMI通道，CEC功能设置为打开状态，按DVD的POWER键开机碟机，通道不跳转至HDMI通道。
> + 0137541: 【51M GTV/欧洲SS客户/ID:221】USB通道播放片源1920x1080（12.2Mbps）（AVC）(AAC,AC-3)(219M).MKV,三个音轨切换播放，出现无声音现象。
> + 0137543: 【51M GTV/欧洲SS客户/ID:221】HDMI1通道接soundbar,USB通道播放片源1920x1080（18Mbps)（MPEG2）(AC-3)(231M).mpg,按遥控器左右方向键后，soundbar出现无声音现象。
> + 0137446: 【41A GTV/Acer客户/ID:101】HDMI测试，只做H14b但同时要保留HDR特性，需要在DUT的EDID1.4中添加HDR模块
> + 0137148: 【GTV_51M/台湾松下/PID112】升级软件后，预置频道，首次和第二次皆无效，第三次才预置成功
> + 0137358: [id139,V10041]P模式按power键关机-》等待20秒开机，操作串口，TV不响应
> + 酒店模式添加断电上电/待机清除app 数据功能
> + 0137220: [EU]4K signal is output to the screen when 4K up-conversion playback of FHD Blu-ray Disc is performed via HDMI.
> + 0137198: [EU]HDR content in HDMI input is played as SDR.
> + 0137232: 【阿联酋EVVOLI/2841A/ID:322】分辨率切换到480i59时，会花屏一下。
> + USB抓log改善措施，添加tv_info.bin TV 信息显示
> + sharp logo错问题复盘添加USB Upgrade 升级成功标识 及串口检测
> + GTV 工厂菜单添加USB DB TO TV 选项
> + LED 松下远场语音恒嘉生产语音功能开发
> + 0136904: 【41A_GTV/印度A客户ID:103】进入Settings-inputs-应去掉HDMI Signal Formart，同RT2841A设计
> + 0136615: 【51M-GTV越南C客户/ID:195】TV HDMI1信源连接PHILIPS DVP3690K/93 DVD，CEC为ON，info信息栏设备名称一直变化（见视频）
> + 0136575: 【51M-GTV越南C客户/ID:195】TV HDMI1信源连接带CEC的功放，CEC为ON，功放关机后几秒马上自动开
> + 0136584: 【51M-GTV越南C客户/ID:195】在HDMI1信源连接PHILIPS DVP3690K/93 DVD，切换HDMI signal format时闪烁（见视频）
> + 0136572: 【51M-GTV越南C客户/ID:195】TV HDMI1信源连接带CEC的功放或DVD，CEC为ON，TV开关机功放或DVD不跟随开关机
> + 马来SIPSI 泰国 NBTC认证需要 NBTC认证-GOOGLE TV OAD码流制作
> + 0136565: 【51M-GTV越南C客户/ID:195】在HDMI1信源连接索尼4KDVD BDP-S6700 DVD，CEC为off，TV开关机后画面闪烁卡顿（见视频）
> + 0136103: 【南美M客户/41A/ID:325】HDMI信源接22294仪器切换分辨率屏幕瞬间闪绿屏


***
### 2023年01月
***
> + 0136032: 【51M-GTV/台湾P客户/ID:112】TV ARC接松下B600功放，TV待机再开机后，声音从TV端输出，查看声音输出选项在ARC
> + 0135903: [id139,V10041] 白平衡调试耗时12秒，含信源切换及版本检测等
> + 0135760: [51M-GTV 哥伦比亚K客户 ID185] 预制HZ_ODM频道后ATV无节目
> + 0135658: [id139,V10041] P模式下，home页面断上电应要回到上一信源
> + 0135851: 【51M/印度P客戶/ID:138】某一信源切换overscan时，会影响另一信源的overscan值
> + 0135368: 【41A GTV/EU SS客户/ID:110】连接ARC功放，连续执行声音设置复位2次，功放会被Mute
> + 0135364: 【41A GTV/EU SS客户/ID:110】EDID制造年份为2022年，应改为2023年
> + 0135341: 【41A GTV/EU SS客户/ID:110】Contrast+9735不能进入工厂菜单
> +	0135317: 【51M/印度P客戶/ID:138】ARC接松下B600功放，USB播放4K超跑、长虹视频，画面、声音卡顿；不接功放播放这些视频仍卡顿
> + 0134975: 【51M/ISDB/ID695】【试产问题】22294输入HDMI 60HZ信源屏天侧闪白点，50HZ正常（ timing 74/73）
> + 0134764: [RT2851M/日本SH客户/ID:416]电视连接CEC设备后，按下遥控器Power键，CEC设备关机，电视仍为开启的状态
> + 0134800: 【51M-GTV/台湾P客户/ID:116】不能通过图像模式>User>对比度+9735进入工厂菜单
> + 0134697: 【41A_GTV/印度A客户ID:001】工厂菜单内project name与FDS内不符（见图）
> + 0134607: 【51M/印度P客戶/ID:139】Shop复位后，默认开机模式为last，请确认
> + 0134603: 【51M/台湾P客戶/ID:116】HDMI播放HDR10+片源，info顯示為HDR10
> + 0134650: 【中美K客户/51M/ID:683】HDMI接22294仪器分辨率识别成DVI格式，画面偏色


***
### 2022年12月
***
> + Led 远场灯光控制功能开发
> + 0134537: [RT2851M/日本I客户/ID:5]在HDMI 1信源下，DVD按播放键，TV会跳至HDMI 3信源
> + 0134535: [RT2851M/日本I客户/ID:5]Device Control Key设为Normal，DVD不响应TV遥控器指令
> + 0134513: 【基础dailybuild 1224 ID45】抄写HDCP2 key后，read key数据与抄写内容不一致
> + 0134467: 【41A_GTV/印度A客户ID:001】工厂菜单有酒店模式，应去掉
> + 0134465: 【41A_GTV/印度A客户ID:001】工厂菜单内的版本号、Clienttype 等修改
> + 0134405: 【RT2851M SINGER客户tconless PID: 647】某些品牌的笔记本电脑接入HDMI后，overscan无作用
> + 0134312: [RT51M 中东LOD项目 ID502]在试产线生产大货机器，发现HDMI分屏问题
> + 0133696: [RT2841A ]CVTE工厂切DVB-S通道后软硬开机问题
> + 0133766: [2841A kalley ID201] 生产问题：首次上电概率性出现机器卡死
> + 0134013: [51M ID654]CVT生产问题：IDL188855A预置CVT工厂信号后，dvb-s没信号，需要重启
> + 0133877: 【dailybuild 版本1208 欧规】WIPE升级-》USB升级-》忙切ID10，开机向导缺少语言选择页
> + 0133747: 【2851M/中东非战略P客户/ID: 301】生产适应性问题 - AT下五分钟会出现屏保，shop复位后等候十分钟没有屏保
> + 0133696: [RT2841A ]CVTE工厂切DVB-S通道后软硬开机问题
> + 0133766: [2841A kalley ID201] 生产问题：首次上电概率性出现机器卡死
> + 0133793: [41A ID149]生产问题：白平衡调试过程中色温坐标不能及时变化，出现G枪会自动压低（概率性），导致产品调试后坐标超规，主观偏色
> + 0133834: 【RT2851M&RT2841A】RTK UI P模式下开关机进入的是主页面而不是信源
> + 0133797: 【台湾松下客户场测问题】GTV工厂菜单隐藏酒店模式
> + 0133789: 【51M/新西兰P客户/ID:127】HDMI3接带dolby vision的松下DVD，再ARC接松下B600功放，播放DVD时，功放无声音输出。
> + 0132794: 【51M-GTV/新西兰P客户/ID:124】MediaPlayer应用下播放HDR视频噪点严重（见图）
> + 0133811: 【南美B客户/51M/ID:261】Media Player应用下播放HDR视频彩色偏淡
> + 50D8300P_HDMI/43D8300P_HDMI HDMI 认证EDID 修改及CEC问题跟进
> + 松下酒店模式实现清除指定应用数据功能实现
> + 日规码流文档整理[https://rd-mokadisplay.tcl.com/kms/pages/viewpage.action?pageId=55457911]https://rd-mokadisplay.tcl.com/kms/pages/viewpage.action?pageId=55457911
> + BTMD ZH ODM 是以800开头的，要重新搜索,并重导到到预制频道
> + 0133412: 【2851M/中国香港战略P客户/ID: 606】生产适应性 - DTMB制式 
> + 0133088: 【ID25 白平衡调试】RGB保存NVM命令执行后会导致下一条命令的响应时间在1s左右，增加白平衡调试时间
> + 0133085: 【ID25 工厂生产适应性测试】工厂适应性测试时，执行ATV(0x20,0,3E/1D）命令，无法跳转到指定台号
> + 0133087: 【ID25 串口指令异常】串口设置亮度异常，实际亮度与输入刻度不符；如0x32 64设置亮度为100，实际只有10左右，无法监控最大亮度
> + 0133086: 【ID25 Shop init复位】复位绿场时左下角"SVN"字段仍显示
> + 0133685: 【51M/香港P客户/id:604】电视接ARC功放（Panasonic HBT100），HDMI3信源接22293，在HDMI3信源下拔插ARC的线，拔掉的瞬间会黑屏，然后恢复正常画面
> + 0130564: 【I客户】-279 No video or audio output after power on with CEC
> + 0133469: 【51M/台灣P客戶/ID:116】ATV或DTV信源下进入频道编辑后，按遥控器返回键时自动退至主页面

***
### 2022年11月
***
> + 0133412: 【2851M/中国香港战略P客户/ID: 606】生产适应性 - DTMB制式
> +	0133432: 41A&51M阿尔及利亚B客户预置频道添加
> + 0133216: 【51M-GTV印度P客户/ID:184】ARC接松下B490功放，Audio output不能自动跳转到ARc选项，手动选项到ARc选项，也不能选成功。
> + RT2851M-GoogleTV机芯维护文档，生产适应性问题文档 [https://rd-mokadisplay.tcl.com/kms/pages/viewpage.action?pageId=68098491]https://rd-mokadisplay.tcl.com/kms/pages/viewpage.action?pageId=68098491
> + 0131945: 【51M-GTV/印度松下/ID:184】工厂自动测试模式下，声音切换到光纤后，光纤没声音输出（问题原因是从喇叭切到光纤后，SPDIF变成了AUTO，要求为PCM）
> + 0131987: 【RT2851M/日本SH客户/ID:416]地面波信源下，连接Yamaha YAS-106 ARC Soundbar，不能自动打开ARC
> + 0132794: 【51M-GTV/新西兰P客户/ID:124】MediaPlayer应用下播放HDR视频噪点严重（见图)
> + 0132369: 【51M/泰国P客戶/ID:107】Google TV模式，DTV已搜台，从ID127切到ID107，工厂菜单NVM后再Shop，开机走完向导后，进入DTV台未清掉。
> + 0132244: 【51M/印度P客戶/ID:139】HDMI1连接LG ARC Soundbar SP8YA，YouTube下，按遥控待机键待机，Soundbar不跟随待机
> + 【51M/印度P客戶/ID:139】HDMI1连接LG ARC Soundbar SP8YA，YouTube下，Output Device从HDMI-ARC设为TV Speaker，会自动回到HDMI-ARC或等待一段时间自动回到HDMI-ARC
> + 0132194: 【51M/台湾P客戶/ID:111】HDMI接带CEC功能的杰科DVD，TV待机后DVD待机，再按DVD开机，有时电视不会跟随开机
> + GTV HDIM 认证EDID 问题解决
> + 0132006: 【SH-32][recurrence]If the "Support HDMI Mode" of HDMI2 is 2.0, there is no description of AAC in the EDID.
> + 0131790: 【51M/泰国P客戶/ID:107】菜单CEC功能中的Tv改为大写
> + 0131511: 【RT2851M/日本SH客户/ID:11】OAD下载完成后，需要重启才会弹出升级提示框
> + 工厂菜单添加AQ/EQ版本号显示功能开发
> + 雷达串口协议定义与及串口获取雷达状态功能开发
> + 0131675: 【51M-GTV/台湾P客户/ID:111】HDMI信源-Menu键-频道和输入来源-外部输入-HDMI CEC控制设置为关，再设置为开时，下面两个子菜单不跟随打开。
> + 0131144: 【51M-GTV/台湾P客户/ID:111】酒店模式，Power Source有cable ATV和Antenna ATV，只保留ATV，与菜单信源一致

***
### 2022年10月
***
> + 0131186: 【SH-32】When Audio Output change from TV to HDMI-ARC, TV change to HDMI too.
> + 0131045: 【51M-GTV/新西兰P客户/ID:101】连接ARC，HDMI-ARC切换到TV Speaker模式，有时不切换到TV音量大小
> + 0131036: 【2841 P升R/新西兰P客户/ID:261】客户问题 - TV locks up when connected to SC-HBT688 soundbar (AC-3 only audio decode).
> + 0130927: 【RT2851M/日本SH客户/ID:416】连接Yamaha ATS-1070，从地面波切至HDMI时，Soundbar source切至HDMI后又切回TV（与客户PQ样机表现不一致）
> + 0130984: 【P升R 新西兰松下RT2841 ID:260】连接soundbar后在TV信源播放，频道直接卡死，然后信源类别TV模块丢失，操作遥控菜单键不能进入三级子菜单选项，只有断电才能恢复。
> + 0130757: 【RT2851M/日本SH客户/ID:416】HDMI连接22293-B，输入Timing 65 PAT 47信号，图像显示不正确
> + 0130836: 【51M-GTV/菲律宾P客户/ID:133】TV连接索尼功放ARC，Audio output切换失败，但是将功放INPUT调至HDMI1，点击HDMI-ARC功放会切换回TV
> + 0130986: 【RT2851M/日本SH客户/ID:416】地面波节目在Digital Audio Out为PCM时，播放流畅，但切至Passthough或Auto时，声音断续
> + 0130695: 【RT2851M/新西兰/ID:101】 开机模式Last，待机状态关开电源电视與用例結果不一致
> + 0130720: 【RT2851M 中东E客户ID:342】HDMI信源下接22294仪器，720p60HZ切到576i50HZ画面瞬间闪花屏
> + 0130694: 【RT2851M/新西兰/ID:101】 开机模式Standby，待机状态关开电源电视與用例結果不一致
> + 0130630: 【51M/印尼松下/id:139】TV连接Jusbe大功放，CEC设置为on，功放跟随电视关机，但是不跟随电视开机
> + 0130659: 【51M/中东P客户/ID:293】usb播放HDR视频噪点严重
> + 0130509: 【41A/中东非P客户/ID:208】连接ARC播放MP3音乐，部分音乐Digital Output为Auto时功放没有声音输出 
> + 0130536: 【RT2851M/日本I客户/ID:5】请提供eARC灰选高亮相关Spec
> + 0130567: 【51M/菲律宾P客戶/ID:133】该项目机型为：43D6130UD， 工厂菜单中为：43D6300


***
### 2022年09月
***
> + 0126490: 【41A/北非S客户/ID:224】HDMI下接22294仪器切分辨率时电视瞬间闪绿画面
> + 香港DTMB 制式功能开发完成
> + 0130427: 【菲律宾P客户/2841a/ID:488】ISDB TV排序问题
> + 0130059: 【51M-GTV/菲律宾P客户/ID:133】TV连接索尼功放，遥控增减音量时，+上下弹跳两次不变，-直接往下减两个音量度
> + 0129965: 【RT2841A ARTEL ID211】导入CVT工厂频道表后，概率出现ATV无信号输出，30%概率
> + 0130194: 【中东A客户/ID:483】ARC正在使用时，电视待机，再开机，概率性出现功放无声（版本V0000751)
> + 0130160: 【51M-GTV/台湾P客户/ID:111】语言为繁体中文，HDMI信源-Menu键-频道和输入来源-外部输入-HDMI CEC 设定菜单-CEC Device List及子菜单未翻译
> + 白平衡检测mgk、playReady key 失败问题分析与解决以及兼容王牌后补串口协议。
> + 工厂菜单整理中间件接口，并说明接口作用
> + 0129937: 【51M/非洲P客户/ID:292】ARC输出时，在media player应用下播放MP4视频或AAC格式音乐，Digital Audio Out设为passthrough时功放无声
> + 0129871: 【41A/ISDB/ID222】无法从eeprom内获取key数据，导致主板没key
> + 0129798: 【泰国松下客户场测问题】复位后，色温超标
> + 0129804: 【51M/B客户/ID253】HDMI CEC功能为开，HDMI1.4与2.0之间相互切换后无像（DVD:SAMSUNG）切换CEC开关也会出黑屏无像（连接YAMAHA功放）
> + 0129526: The Noise of Audio from AMP(connected with ARC) is terrible at digital audio output setting is "auto"
> + 0129290: 【51M/南美M客户/ID:462】ARC输出时，在media player应用下播放MP4视频或AAC格式音乐，Digital Audio Out设为passthrough时功放无声
> + 0129527: The noise of the audio from the AMP(connected with eARC) is terrible.
> + 0129069: 【RT2851M/日本SH客户/ID:416】地面波信源下，连接ARC Soundbar，执行声音复位，将Digital Audio Output设为Passthroug，无声音输出
> + Factory Shop 复位红屏断电导致上电模式异常问题分析与跟进
> + Factory 调用中间接口整理【https://rd-mokadisplay.tcl.com/kms/display/SWHelloWorld/090112-Factory】【https://rd-mokadisplay.tcl.com/kms/pages/viewpage.action?pageId=27980007】
> + 128356: 【伊拉克A/2851M/ID:486】ALLM功能开启后，玩X-BOX游戏（极限竞速：地平线）时画面抖动异常，且界面右边显示重叠
> + 128322:【伊拉克A/2851M/ID:486】玩X-BOX游戏时，游戏机的遥控器返回X-BOX主页时，画面黑屏。
> + 0128459: 【SH-32】ID:11 HDMI 认证 CEC2.0 FAIL，Test HF4-2-4项Fail，对于自身未声明Follow支持的指令，DUT未将其忽略，而是有响应。
> + 0128460: 【SH-32】ID:11 HDMI 认证 CEC2.0 FAIL，Test HF4-2-10项Fail，DUT收到polling message后未回复ACK
> + 0128461: 【SH-32】ID:11 HDMI 认证 eARC FAIL，测试eARC协议时，差分信号的协议测试项结果均为fail，通过示波器查看，发现DUT回传的eARC只有共模信号，没有差分信号。
> + 0128935: TV restart when AV AMP is connected with eARC and POWER OFF->POWER ON


***
### 2022年08月
***

> + 0127185: 【RT2851M/日本SH客户/ID:11】HDMI 1口连接三星功放，将eARC设置为ON后，会出现功放无声现象
> + 0128459: 【SH-32】ID:11 HDMI 认证 CEC2.0 FAIL，Test HF4-2-4项Fail，对于自身未声明Follow支持的指令，DUT未将其忽略，而是有响应。
> + 0128460: 【SH-32】ID:11 HDMI 认证 CEC2.0 FAIL，Test HF4-2-10项Fail，DUT收到polling message后未回复ACK
> + 0128461: 【SH-32】ID:11 HDMI 认证 eARC FAIL，测试eARC协议时，差分信号的协议测试项结果均为fail，通过示波器查看，发现DUT回传的eARC只有共模信号，没有差分信号。
> + 128356: 【伊拉克A/2851M/ID:486】ALLM功能开启后，玩X-BOX游戏（极限竞速：地平线）时画面抖动异常，且界面右边显示重叠
> + 128322:【伊拉克A/2851M/ID:486】玩X-BOX游戏时，游戏机的遥控器返回X-BOX主页时，画面黑屏。
> + 0125965: 【51M/新西兰P客户/ID:183】客户问题：Switch on Dolby E-AC-3 Atmos soundbar (SC-HTB700) with TV CEC causes Android OS to re-boot. Sometimes th
> + 0125964: 【51M/新西兰P客户/ID:183】客户问题：Time to switch on Dolby AC-3 soundbar by CEC is approximately 30 seconds. Audio is initially PCM for sev
> + 0126476: 【SH-32】Audio Copy protection Err:The sound of DVD Audio and Super Audio CD is output from SPDIF/Optical
> + 0128304: 【2851M/印度Acer/ID:450】ARC功能不起作用。
> + 0128263: 【PANASONIC_smoketest_002】声音模式调节到ByPass，播放“Spider-Man:Homecoming”和”Blade Runner 2049”,功放上均无显示Dolby 标识
> + 0125968: 【51M/新西兰P客户/ID:183】客户问题：HDMI ARC output with eARC ON results in audio dropouts when Digital Audio Out is AUTO on NETFIX program
> + 0125965: 【51M/新西兰P客户/ID:183】客户问题：Switch on Dolby E-AC-3 Atmos soundbar (SC-HTB700) with TV CEC causes Android OS to re-boot. Sometimes th
> + 0127494: 【51M/台灣P客戶/ID:147】HDMI不能播放HDR10+片源
> + 0128070: 【51M-GTV/台湾P客户/ID:147】语言为繁体中文，HDMI信源-Menu键-频道和输入来源-外部输入：HDMI信号格式：显示为：standard format等
> + 0128076: 【51M-GTV/台湾P客户/ID:147】语言为繁体中文，HDMI信源-Menu键-频道和输入来源-外部输入-HDMI CEC控制为关时，下面两个菜单仍可以操作。
> + 0128074: 【51M-GTV/台湾P客户/ID:147】语言为繁体中文，HDMI信源-Menu键-频道和输入来源-外部输入-HDMI CEC设置：设备自动开机和电视自动关机应与51M描述一致。
> + 0128073: 【51M-GTV/台湾P客户/ID:147】语言为繁体中文，HDMI信源-Menu键-频道和输入来源-外部输入：HDMI CEC 设定菜单缺少：CEC设备列表
> + 0128072: 【51M-GTV/台湾P客户/ID:147】语言为繁体中文，HDMI信源-Menu键-频道和输入来源-外部输入：HDMI CEC设置改为：HDMI CEC 设定
> + 0127452: 【51M/台灣P客戶/ID:147】HDMI信源下，進入Menu>頻道和輸入來源>Inputs>HDMI CEC設置，CEC控制設為關，設備自動關機、電視自動開機不跟隨灰選
> + 日本S客户OAD制作 OAD 码流制作
> + 日本4K转2k  0127745: HDMI recorder Audio Multiplex NG; When choise HDMI recorder unable to switch audio setting via TV RC "音声切換" key
> + 日本4K转2k 	0127762: CEC command does not work properly"Unmatch our intenal list"
> + 日本I客户【0127787】预抄写软件测试 4K HDMI 画面显示异常，无声音
> + P客户2023项目 Hotel mode 功能评估
> + 【0127545】51M 工厂模式下显示MEMC版本号
> + 泰国和马来西亚2841A-OAD码流制作
> + 产线小米盒子HDMI 花屏问题分析与跟进
> + Factory 添加Light Sensor Curve 功能
> + Factory 添加显示MEMCV信息及中间件添加MEMCV接口
> + 眼图功能的开发与验证
> + 申请新增Audioelec工厂预置频道及db同步到41A/51M各分支
> + 2851M Google TV 国内DTMB预制频道开发功能
> + P升R casper升级DB数据结构不一致问题 Factory 第一次 Shop解决方案

***
### 2022年07月
***
> + RT2851M机芯生产适应性调试说明文档编写
> +  RT2851M-泰国/马来OAD码流制作与验证
> + 【0126733】工厂PS模式下，串口发送shop指令，TV会退出PS模式且不会执行shop
> + 【0126638】【2851M/欧洲P客户/ID:176】酒店模式中图像/声音模式无Filmmaker Mode、Power Channel中无数字排序
> + 【0126613】【2841A/中东非战略P客户/ID:208】客户问题 - Hotel Menu中Power Source有HDMI3項目,應該刪除.
> + 【0126508】中东客户问题 - Hotel Menu问题-Channel Lock無LAST MEMORY.Powe Source 無AV MODE
> + 41A生产反馈在config 设置STB,Shop 红屏断电后概率上电模式为ON问题
> + Google TV 工厂相关功能生产适应性测试
> + 松下远场语音新增远场灯光测试等串口问题开发。
> + 日规生产反馈白平衡6月初正常，7月内置白场第一次黑屏问题分析
> + 【0121434】 欧洲STRONG客户ID167 Factory data copy 无tv.db 文件问题。
> + 【0126086】日本SH客户 SPEC需求不支持Dolby vision，电视可播放Dolby vision, 但播放后图像异常
> + 工厂菜单眼图UI讨论与界面实现。
> + 【0125981】RT2851M/日本SH客户/ID:11白平衡调试内置白场调试第一次打开白场失败
> + 【0124956】51 P升R，工厂模式通过串口预制惠州频点后，再发指令如切信源，TV自动退出PS模式问题分析。
> + 【0125909】分析六合一自研主板p模式下串口通信，TV重启问题。



***
### 2022年06月
***
> + 工厂菜单添加P模式日志保存功能。
> + 松下ODM机型51M/41A机芯Menu键值冲突问题确认与跟进。
> + 眼图添加UI设置及展频参数设置
> + 【0124524】【0124525】中东非战略P酒店模式下的Pic sound Mode 与TV 设置下的pic sound Mode 不一致问题。
> + 欧规STRONG客户出现USB，串口工具，工厂在线抄key软件无法找CI key问题
> + 乌兹别克A客户生产KEY软件上切换ID会出现导致P模式消失，CI key丢失问题
> + 远场语音工作状态CRC值不对问题解决
> + CVT生产适应性反馈问题验证与解决 
> + 申请并添加印尼松下PGI工厂预制频道，并且同步各分支DB数据
> + 松下菲律宾项目生产适应性测试反馈问题验证以及分析解决
> + 41A/51M工厂遥控器打开P模式串口命令工具无法打开问题分析与解决



***
### 2022年05月
***
> + 0122707: 【2851 哥伦比亚 ID：782】工厂菜单中shop后不会进入绿屏，而是进入google登录界面，遥控器失效
> + 南美solink客户预置频道数据导入以及各个分支db同步
> + 【0124573】日规项目HDMI信源下，P模式断电上电后，未能重新进入HDMI信源下，而是跳转到DTV信源
> + 2851M ATSC制式CVT预制频道添加，以及同步到各个分支
> + 红屏断言添加/data/anr与/data/tombstones U盘拷贝功能
> + 【0124484】生产问题出现HDMI插上小米盒子画面花屏问题产线跟线分析与问题跟进
> + 【0124441】日规在Pixela向导界面预制频道时不关闭Pixela界面问题解决
> + 泰霖添加设置背光和led灯的串口指令问题
> + 【0124240】日规项目，在P模式下，已经预制频道后，升级软件，添加节目不清除功能
> + 【0123696】波兰工厂反馈松下遥控红外发码的IR factory Command 无作用
> + 【0124124】试产线反馈"调试白平衡监测最大亮度白场会消失导致白平衡调试不过"
> + 【0121560】中美ID:137，白平衡调试第一次打开白场黑屏，第二层打开恢复正常白场