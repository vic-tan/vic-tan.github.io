...
***
* 问题分析
    > 开始时白平衡机器WB init delay设置为200ms异常，冷色温偏低，设置500ms 全部通过，通过白平衡发送串口日志，发现切内置白场花花时间用了0.352s，正常情况只需要0.03秒左右，然后再次对比正常白平衡数据与异常白平衡数据，发现异常数据在调用白平衡初始化之前先用调用进入HDMI 信源串口，而正常数据在初始白平衡之前没有调用，而是在整个白平衡调试完成后在去调用进入信源指令。所以初步怀疑是在初始化白平衡之前高用进入HDMI信源时占用了部分时间，导致白场还未完全显示出来仪器便开始进行了对白平衡调试，导致白平衡冷色温偏低。跟产线人员沟通，先把机架进入HDMI信源先不勾选，还是200ms。结果测试全部通过，所以基本确定猜想正常。
* 解决方法 
